...
const ProtectedRoute = ({ children, allowedRoles }: ProtectedRouteProps) => {
  ...
  // Authority route: only allow the specific authority emails
  if (allowedRoles?.includes("authority") && allowedRoles.length === 1) {
    const userEmail = user.email?.toLowerCase();
    if (!userEmail || !AUTHORITY_EMAILS.includes(userEmail)) {
      return (
        <div className="flex min-h-screen flex-col items-center justify-center bg-background gap-4">
          <div className="rounded-xl bg-card p-8 shadow-card text-center max-w-md">
            <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-destructive/10">
            ...
            <h2 className="text-xl font-bold text-foreground">Access Denied</h2>
            <p className="mt-2 text-sm text-muted-foreground">Only authorized authority accounts can access this page.</p>
            <a href="/" className="mt-4 inline-block rounded-xl bg-primary px-6 py-2.5 text-sm font-semibold text-primary-foreground transition hover:brightness-110">Go Home</a>
          </div>
        </div>
      );
